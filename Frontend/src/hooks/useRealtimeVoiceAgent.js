...
export default function useRealtimeVoiceAgent(wsUrl = 'wss://akuh-voice-agent.onrender.com') {
  ...
  // Start continuous audio recording from user microphone
  const startMicrophoneStream = useCallback(async () => {
    try {
      ...
      scriptNode.onaudioprocess = (audioProcessingEvent) => {
        ...
        // Echo Suppression: Do NOT record or transmit microphone audio while AI is speaking!
        ...
        // Noise Gate: Drop background static / laptop fan hum when user is not speaking
        if (rms < 0.025) {
          setVolumeLevel(0);
          return;
        }

        // Throttle volume calculation to avoid React re-render lag during speech
        const now = Date.now();
        ...
        const resampledData = resampleFloat32(inputData, audioCtx.sampleRate, 24000);
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'ai_transcript_done':
            // Audio finishes playing soon after
            break;
  ...
  }, [wsUrl, stopAudioPlayback, startMicrophoneStream, playAudioChunk, stopMicrophoneStream]);
